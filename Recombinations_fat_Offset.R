#load data
PhasMisFat3<-read.table('Output_files/W1WC/W1_2/Offset/PhasMisFat2.txt') ##output file Origin_alleles_fat.R
RecFat2<-as.matrix(PhasMisFat3)
map<-read.table("Input_files/Chr1_map2b.txt", header=TRUE) ##output file Phasing_mistakes.R
ped<-read.table("Input_files/ped5b.txt") ##output file Phasing_mistakes.R
...
seventy<-c(0, seq(cl/2,ncol(RecFat2)-cl,by=cl),ncol(RecFat2))
...
for (j in 1:nrow(RecFat2)){
for (k in 1:(ncol(RecFat2)/cl+1)){
inf_list[[k]]<-table(factor(RecFat2[j,(1+seventy[k]):(seventy[k+1])],lev=c(0,1,2,5,9)))}
inf_list2[[j]]<-inf_list}
##estimate fraction of informative markers per core for each individual
inf_list3<-lapply(inf_list2, function(x){
lapply (x, function(y){
(y[[2]]+y[[3]])/sum(y)})})
inf_vec<-lapply(inf_list3, unlist)
##correct amount of recombinations for uninformative markers
recombcor<-list()
for (i in 1:length(inf_vec)){
recombcor[[i]]<-recomb3b[[i]]/inf_vec[[i]]}
recombcor<-rapply(recombcor, f=function(x) ifelse(is.nan(x),0,x),how="replace")
recombcor2<-lapply(recombcor, function(x){
as.data.frame(t(as.data.frame(x)))})
## corrected location recombinations
rec_per_core2<-colSums(rbindlist(recombcor2, fill=T), na.rm=T)
...
rec_per_Mb<-as.data.frame(rec_per_Mb)
rec_per_Mb2<-rec_per_core2/core_length*1000000
rec_per_Mb2<-as.data.frame(rec_per_Mb2)
#middle of cores
...
colnames(homozygotes)<-c('father','homozygosity')
recomb_fat<-merge(recomb_fat,homozygotes,by='father')   #homozygotes per father
recomb_fat[,5]<-numeric(nrow(recomb_fat))
for(i in 1:nrow(recomb_fat)){
recomb_fat[i,5]<-length(which(recomb_fat[,1]==recomb_fat[i,1]))} #number of repeated observations per father
colnames(recomb_fat)<-c('father', 'individual','recombinations','homozygosity','repeats')
...
###############figures#################
pdf('Output_files/W1WC/W1/Offset/hist_rec_per_individual.pdf')
par(mfrow=c(1,1))
...
pdf('Output_files/W1WC/W1/Offset/hist_rec_per_father.pdf')
par(mfrow=c(1,1))
...
pdf('Output_files/W1WC/W1/Offset/inf_markers_vs_rec.pdf')
...
###########tables###########
##recombinations
capture.output(tor2, file='Output_files/W1WC/W1/Offset/tor2.txt')
capture.output(recomb1b, file='Output_files/W1WC/W1/Offset/recomb1b.txt')
write.table(recomb1c, 'Output_files/W1WC/W1/Offset/recomb1c.txt')
capture.output(tor3b, file='Output_files/W1WC/W1/Offset/tor3b.txt')
capture.output(recomb3b, file='Output_files/W1WC/W1/Offset/recomb3b.txt')
write.table(recomb3c, file='Output_files/W1WC/W1/Offset/recomb3c.txt')
###per father
write.table(recomb_fat, 'Output_files/W1WC/W1/Offset/recomb_mot.txt')
write.table(meansum, 'Output_files/W1WC/W1/Offset/meansum.txt')
##locations
write.table(rec_per_core, 'Output_files/W1WC/W1/Offset/rec_per_core.txt')
write.table(rec_per_core2, 'Output_files/W1WC/W1/Offset/rec_per_core2.txt')
write.table(rec_per_Mb, 'Output_files/W1WC/W1/Offset/rec_per_Mb.txt')
write.table(rec_per_Mb2, 'Output_files/W1WC/W1/Offset/rec_per_Mb2.txt')
capture.output(inf_vec, file='Output_files/W1WC/W1/Offset/inf_vec.txt')
